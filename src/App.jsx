...
export default function App() {
  ...
  const handleLogin = async () => {
    ...
    const userData = {
      ...
      isAdmin: data.is_admin, isTeamLead: data.is_team_lead, teamId: data.team_id
    }
...
function TeamMembersManage({ teamId, allMembers, refresh, canManage }) {
  ...
  const availableMembers = allMembers.filter(m => !m.team_id && !m.is_admin)

  const addExisting = async () => {
    if (!existingSelect) return alert("Member select karein!")
    await supabase.from('members').update({ team_id:teamId }).eq('id', existingSelect)
  ...
  const createNew = async () => {
    ...
    const { error } = await supabase.from('members').insert({
      ...
      is_admin:false, is_team_lead:false, team_id:teamId
    })
        ...
        <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, padding:14, marginBottom:12 }}>
          {availableMembers.length === 0 ? (
            ...
            <>
              ...
              <button onClick={addExisting} style={{ background:C.primary, border:"none", color:"#fff", padding:"7px 16px", borderRadius:6, fontSize:12, cursor:"pointer", fontWeight:600 }}>Add to Team</button>
            </>
          )}
        </div>
      )}
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
        {teamMembers.length === 0 ? (
          ...
          <div key={m.id} style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:8, padding:"10px 14px", display:"flex", alignItems:"center", gap:12 }}>
            ...
            {canManage && !m.is_team_lead && (
              <button onClick={()=>removeFromTeam(m.id)} style={{ background:"transparent", border:`1px solid ${C.border}`, color:C.danger, fontSize:11, padding:"5px 10px", borderRadius:6, cursor:"pointer" }}>Remove</button>
            )}
          </div>
        ))}
      </div>
  ...
  return (
    <div>
      ...
      {Object.keys(niches).length > 0 && (
        <>
          <h3 style={{ color:C.text, fontSize:14, fontWeight:600, marginBottom:14, textTransform:"uppercase" }}>Niche Breakdown</h3>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:10 }}>
            {Object.entries(niches).sort((a,b)=>b[1]-a[1]).map(([n, c]) => (
              <div key={n} style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:10, padding:"12px 14px" }}>
                <p style={{ color:C.text, fontSize:14, fontWeight:600 }}>{n}</p>
                <p style={{ color:C.textMuted, fontSize:12 }}>{c} accounts</p>
              </div>
            ))}
          </div>
        </>
...
function AdminMembers({ data, refresh }) {
  ...
  const addMember = async () => {
    ...
    const { error } = await supabase.from('members').insert({ id, name:form.name, email:form.email.toLowerCase(), password, role:form.role, checkin_time:form.checkin_time, avatar, is_admin:false, is_team_lead:false, team_id:form.team_id||null })
    if (error) return alert("Error: " + error.message)
    await supabase.from('member_stats').insert({ member_id:id, late_count:0, strikes:0 })
  ...
  const saveEdit = async () => {
    ...
    await supabase.from('members').update({ name:editForm.name, email:editForm.email.toLowerCase(), password:editForm.password, role:editForm.role, checkin_time:editForm.checkin_time, avatar, team_id:editForm.team_id||null }).eq('id', editId)
    setEditId(null); setEditForm(null)
    refresh()
  }
...
function TeamLeadDashboard({ user, onLogout }) {
  ...
  const refresh = useCallback(async () => {
    ...
    const [{ data:t }, { data:allMems }, { data:tasks }, { data:att }, { data:reps }, { data:stats }, { data:rc }] = await Promise.all([
      supabase.from('teams').select('*').eq('id', user.teamId).maybeSingle(),
      supabase.from('members').select('*').eq('is_admin', false).order('created_at'),
          ...
          <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:12 }}>
              ...
              <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:18 }}>
                ...
                <p style={{ color:C.warning, fontSize:24, fontWeight:700 }}>{myData.tasks.filter(t=>t.status!=="done").length}</p>
              </div>
              <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:12, padding:18 }}>
                <p style={{ color:C.textMuted, fontSize:12 }}>My Attendance</p>
                <p style={{ color:C.success, fontSize:18, fontWeight:700 }}>{myData.attendance[today()] ? "✓ Present" : "✗ Not yet"}</p>
              </div>
            </div>
          </div>
        )}
...
function MemberDashboard({ user, onLogout }) {
  const [tab, setTab] = useState("home")
  const [data, setData] = useState({ tasks:[], attendance:{}, reports:{}, stats:{lateCount:0,strikes:0}, reportComments:{}, team:null, teamMembers:[] })
  const [loading, setLoading] = useState(true)

  const refresh = useCallback(async () => {
    const [{ data:tasks }, { data:att }, { data:reps }, { data:stats }, { data:rc }, teamRes, tmRes] = await Promise.all([
      supabase.from('tasks').select('*').eq('assigned_to', user.id).order('created_at'),
      supabase.from('attendance').select('*').eq('member_id', user.id),
      supabase.from('reports').select('*').eq('member_id', user.id),
      supabase.from('member_stats').select('*').eq('member_id', user.id).maybeSingle(),
    ...
    const repMap = {}; (reps||[]).forEach(r=>{ repMap[r.date]={tasksCompleted:r.tasks_completed,hoursWorked:r.hours_worked,blockers:r.blockers,notes:r.notes} })
    const rcMap = {}; (rc||[]).forEach(c=>{ if(!rcMap[c.report_date]) rcMap[c.report_date]=[]; rcMap[c.report_date].push({author:c.author, text:c.text}) })
    setData({ tasks:tasks||[], attendance:attMap, reports:repMap, stats:stats?{lateCount:stats.late_count, strikes:stats.strikes}:{lateCount:0,strikes:0}, reportComments:rcMap, team:teamRes.data, teamMembers:tmRes.data||[] })
    setLoading(false)
  }, [user.id, user.teamId])
